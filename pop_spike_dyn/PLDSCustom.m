...
yDim    = 150;											    	% observed dimension = no of neurons
...
T       = 150;												% no of time bins per trial; here a time step is approx 10ms 
Trials  = 100;		    										% no trials
maxIter = 100;

data = load("PSTH_format_CSM009_NoEx_BSLN_0_20191108_tilt.mat");
relative_response = data.psth_struct.LEFT.event_9.relative_response;
for tr = 1:Trials
    seq(tr).y = transpose(reshape(relative_response(tr, :), [150, 14]));
    seq(tr).T = T;
end
